...
| --- | --- | --- | --- | --- | --- |
| 1 | Authentication / Users | `/`, `/join` | [entry.tsx](src/screens/entry.tsx), [SessionProvider.tsx](src/domain/data/SessionProvider.tsx) (`useSession`, `useUserId`, `SessionGate`) | [seams.test.ts](src/domain/seams.test.ts) | **Partial** — session identity and the sign-in surface exist; there is no credential check or token. See Known limitations. |
